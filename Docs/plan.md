...
**进度（2026-09-22）**：状态机/退避策略/连接链路（`SdkCameraSession`，照 Demo
ConnectionViewModel 抄录）+ SessionEvent 广播 + FGS 持有会话 + SignApp 初始化
SDK 已落地，单测 32 绿。集成中发现并修正状态机设计：被动断连的触发面须覆盖
已进入授权的连接态（Authorizing/Preparing），且重试计数改为与退避序列一一对应
（原从状态形状推导会在重试失败路径上错位）。待办：UI（扫描/连接入口）+ 真机
矩阵验证（需装机，同时补 P1 装机检查）。
