...
**进度（2026-09-23，取流链路真机验证通过）**：`FrameAssembler`（8 条契约单测）
+ `ChunkIngestQueue`（过载单次报告/丢弃/恢复 6 条单测）+ `SdkCameraSession` 开流接线
（连接后立即开流 → 入口队列 → 聚合 → `getPreviewParams` 查询进 Streaming）。
真机实测：640×360@30 H.264、~9KB/帧、稳定 30fps、取流期间零断连/零过载、
相机不再休眠。真机驱动修正：**GO 3S 不触发 `onParamsChanged`**（全程零回调），
...
2. 30 分钟持续取流观察按用户决定不单独空跑（2026-09-23）：改为在解码链真机联调
   的长会话中顺带观察持续稳定性与温升。
3. 4K 档实测收到的 VIDEO 流仍是低分辨率流（~9KB/帧与 4Mbps 档吻合），
   主流/副流的对应关系待解码链接入 SPS 后核实。
