...
此接口接收 App 已切好的**一个句子段**内的有序手势结果。每段手势可提交 CV 的前 1–3 个词候选，按排名排列；`score` 可省略，即使提供也是未校准 softmax，不能当正确概率。有效请求会调用 LLM，从以下五句中强制选择一句：
...
```json
{
  ...
  "revision": 1,
  "sentence": "我想回家",
  "alternatives": ["我想回家", "你一定可以", "你要照顾好自己", "祝大家新年好", "我们只是好久不见"],
  "status": "CANDIDATE",
  "needsConfirmation": true
}
```

HTTP 200 时 `status` 固定为 `CANDIDATE`，`sentence` 必定是五句之一；`alternatives` 列出全部五句（包含已选句），供 App 核对界面使用。LLM 会参考候选词顺序与排名，即使证据不足也必须选一句。因此选句不代表识别可靠；`needsConfirmation` 恒为 true，App 必须展示给用户确认，不能直接自动播报。若模型超时、不可用、返回 null 或越界句子，接口返回相应错误而不会假造一个成功响应。当前模型对胸前视角的同场次抽查为 13/19，且缺少非目标动作拒识；五句组合效果尚未独立验证。
...
LLM_BASE_URL 为 API 前缀，程序追加 `/chat/completions`。使用 Bearer Key，提交 model、stream=false、system/user messages；读取 choices[0].message.content 的严格 JSON。`/v1/polish` 的模型输出必须为 polishedChinese/translations/issues；`/v1/compose-signs` 的模型输出只允许 sentence（五句之一，不能为 null）。均不接受 Markdown。不强制供应商 JSON mode，无自动本地降级。
