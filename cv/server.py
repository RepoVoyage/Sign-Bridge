...
class RecognizeHandler(BaseHTTPRequestHandler):
    def reply(self, status, body):
        ...
        if self.path == '/v1/recognize':
            print(json.dumps({
                'event': 'cv_recognize_response',
                'http_status': status,
                'elapsed_ms': round((time.monotonic() - self.request_started) * 1000)
                if hasattr(self, 'request_started') else None,
                'request': getattr(self, 'request_meta', {}),
                'response': body,
    ...
    def do_POST(self):
        ...
        try:
            ...
            self.request_meta['video_sha256'] = hashlib.sha256(data).hexdigest()
